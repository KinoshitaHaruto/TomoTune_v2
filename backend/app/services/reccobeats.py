"""
ReccoBeats API を使って Spotify 楽曲の audio features を取得するサービス。

フロー:
  Step1: GET /v1/track?ids={spotify_track_id}  → ReccoBeats 内部 ID を取得
  Step2: GET /v1/track/{recco_id}/audio-features → 特徴量を取得

戻り値:
  HIT  → (features: dict, "reccobeats")
  MISS → (None, "miss")
"""
import logging
import httpx
...
async def get_audio_features(spotify_track_id: str) -> tuple[dict | None, str]:
    """
    Spotify track ID から ReccoBeats 経由で audio features を取得する。
    Returns:
        (features_dict, "reccobeats") on success
        (None, "miss") on failure
    """
